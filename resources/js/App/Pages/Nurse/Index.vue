<script setup>
  ...
  import NurseInfoCard from "@/App/Pages/Nurse/Components/NurseInfoCard.vue";
            ...
            <div v-for="data in dataRows.data" class="col-12 col-md-4">
                ...
                >
                    <div class="border-top text-center gap-1 hstack mt-2 pt-3">
                        <edit-button-link
                            v-if="userHasPermission('edit nurses') && data.deleted_at === null"
                            preserve-state
                            preserve-scroll
                            :url="appRoute('nurses.edit', {id: data.id})"
                            :resource="viewElements.resource"
                        />

                        <delete-button-link
                            v-if="userHasPermission('delete nurses') && data.deleted_at === null"
                            preserve-state
                            preserve-scroll
                            :url="appRoute('nurses.delete', {id: data.id})"
                            :resource="viewElements.resource"
                            type="button"
                        />
                    </div>
